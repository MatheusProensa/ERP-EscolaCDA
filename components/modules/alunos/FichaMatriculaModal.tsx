    ...
    <span className="inline-flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[13px]">
      {opcoes.map((o) => (
        <button
          ...
          className={
            valor === o.valor ? "font-bold text-cda-blue" : "text-cda-text3"
          }
        >
...
function TituloSecao({ children }: { children: React.ReactNode }) {
  return (
    <p className="mb-1.5 mt-5 text-[13px] font-bold uppercase tracking-wide text-cda-text3 first:mt-0">
      {children}
    </p>
  );
}
...
function CamposResponsavel({
  ...
  r: Responsavel;
  set: (r: Responsavel) => void;
}) {
  const upd = (campo: keyof Responsavel) => (v: string) =>
    set({ ...r, [campo]: v });
  return (
    <>
      <Linha>
      ...
      <Linha>
        ...
        <Celula label="Celular">
          <CampoTexto
            value={r.telefoneCelular}
            onChange={upd("telefoneCelular")}
          />
        </Celula>
        <Celula label="Tel. Comercial">
          <CampoTexto
            value={r.telefoneComercial}
            onChange={upd("telefoneComercial")}
          />
        </Celula>
      </Linha>
    </>
...
export function FichaMatriculaModal({
  ...
  /** Controlado de fora agora — o gatilho virou um item do menu em
   * FichaMatriculaAcoes, junto com "Baixar PDF". */
  open: boolean;
  onClose: () => void;
  alunoId: string;
  matriculaId: string;
  ...
  const [obsBicoMamadeira, setObsBicoMamadeira] = useState(
    obsBicoMamadeiraInicial,
  );
  const [jaFrequentouEscola, setJaFrequentouEscola] = useState(
    jaFrequentouEscolaInicial,
  );
  const [duracaoEscolaAnterior, setDuracaoEscolaAnterior] = useState(
    duracaoEscolaAnteriorInicial,
  );
  const [rotinaSonoAlimentacao, setRotinaSonoAlimentacao] = useState(
    rotinaSonoAlimentacaoInicial,
  );
  const [brincadeirasPrediletas, setBrincadeirasPrediletas] = useState(
    brincadeirasPrediletasInicial,
  );
  const [reacoesContrariado, setReacoesContrariado] = useState(
    reacoesContrariadoInicial,
  );
  ...
  return (
    ...
    >
      <p className="mb-3 text-sm text-cda-text3">
        É o próprio documento — edita direto em cima. O que já tá no cadastro
        aparece fixo; só o que falta vira campo.
      </p>
      ...
      <div className="overflow-hidden rounded-xl border border-cda-border shadow-sm">
        ...
        <div className="max-h-[72vh] overflow-y-auto bg-white px-6 py-3">
          <p className="mb-3 text-[18px] font-bold text-cda-text">
            FICHA DE MATRÍCULA
          </p>
          ...
          <Linha>
            <Celula label="Obs. (bico/mamadeira)" peso={3}>
              <CampoTextoLivre
                value={obsBicoMamadeira}
                onChange={setObsBicoMamadeira}
              />
            </Celula>
          </Linha>
          <Linha>
            <Celula label="Já frequentou outra escola?" peso={1.6}>
              <ToggleSimNao
                value={jaFrequentouEscola}
                onChange={setJaFrequentouEscola}
              />
            </Celula>
            <Celula label="Duração">
              <CampoTexto
                value={duracaoEscolaAnterior}
                onChange={setDuracaoEscolaAnterior}
              />
            </Celula>
          </Linha>
          <Linha>
            <Celula label="Questões de sono/alimentação" peso={3}>
              <CampoTextoLivre
                value={rotinaSonoAlimentacao}
                onChange={setRotinaSonoAlimentacao}
              />
            </Celula>
          </Linha>
          <Linha>
            <Celula label="Brincadeiras prediletas" peso={3}>
              <CampoTextoLivre
                value={brincadeirasPrediletas}
                onChange={setBrincadeirasPrediletas}
              />
            </Celula>
          </Linha>
          <Linha>
            <Celula label="Reações quando contrariado(a)" peso={3}>
              <CampoTextoLivre
                value={reacoesContrariado}
                onChange={setReacoesContrariado}
              />
            </Celula>
          </Linha>
          <p className="mt-1.5 text-[12px] text-cda-text3">
            Saúde, alergias e medicação vêm do cadastro (aba Censo) — entram na
          ...
          {pessoasAutorizadas.map((p, i) => (
            <Linha key={i}>
              <Celula label={`${i + 1}) Nome`} peso={1.6}>
                <CampoTexto
                  value={p.nome}
                  onChange={(v) => setPessoa(i, "nome", v)}
                />
              </Celula>
              <Celula label="Parentesco">
                <CampoTexto
                  value={p.parentesco}
                  onChange={(v) => setPessoa(i, "parentesco", v)}
                />
              </Celula>
              <button
                type="button"
          ...
          <div className="mt-4 border-t border-cda-border pt-3">
            <p className="text-[12.5px] leading-relaxed text-cda-text2">
              Eu autorizo a utilizar a imagem do aluno (em fotos e vídeos) em
              materiais de divulgação institucional e pedagógica da escola,
              exclusivamente para fins de promoção de suas atividades, podendo
              vinculá-las em suas redes sociais, website oficial, material
              impresso e outdoor.
            </p>
            <div className="mt-1.5">
              <ToggleSimNao
                value={autorizacaoImagem}
                onChange={setAutorizacaoImagem}
              />
            </div>
          </div>
          <div className="h-3" />
