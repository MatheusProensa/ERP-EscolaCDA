 ...
 * enxuta de eventos embaixo de cada mês.
 *
 * Histórico (out/2026, pra não repetir a volta e meia): a 1ª versão trazia
 * TODOS os eventos do banco (~380/ano, a maioria coisa interna tipo "PRAZO
 * PLANO EI"), muito mais cheio que a referência do dono (~20 no ano todo,
 * só feriado/recesso). O dono pediu pra "tirar os eventos" (2x — achei que
 * era pra tirar tudo, cheguei a remover até o destaque amarelo do dia), até
 * mandar de novo a referência dizendo "exatamente assim": o formato CERTO é
 * bolinha + listinha, só que com os eventos FILTRADOS pra categoria
 * Recesso/Feriado (é o filtro que a rota de API aplica antes de chamar essa
 * função — ver app/api/calendario/pdf/route.ts) — bate exatamente com o que
 * a referência mostra.
 *
 ...
 * "+N eventos" se não couber (a rota já filtra pra só feriado/recesso antes
 * de chegar aqui, então isso deve ser raro na prática). */
function desenharCardMes(
  ...
  // título em 1 linha só (trunca com reticências se não couber). Corta em
