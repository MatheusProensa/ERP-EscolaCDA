    ...
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nova avaliação nutricional">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <Input label="Data da avaliação" name="data" type="date" required defaultValue={hojeISO} max={hojeISO} />
          <div className="grid grid-cols-2 gap-3">
            <Input label="Peso (kg)" name="pesoKg" type="number" step="0.01" min="0" required placeholder="Ex.: 14.2" />
            <Input label="Altura (cm)" name="alturaCm" type="number" step="0.1" min="0" required placeholder="Ex.: 92.5" />
          </div>
